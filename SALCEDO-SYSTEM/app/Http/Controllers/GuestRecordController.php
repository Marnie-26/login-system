...
class GuestRecordController extends Controller
{
    ...
    public function view_guest_record(){
    ...
    public function delete_guest_record($id){
        ...
        if ($guest) {
            $guest->delete();
            return redirect('/view-guest-record')->with('success', "Visitor's record deleted successfully.");
        } else {
            return redirect('/view-guest-record')->with('error', "Visitor's record not found.");
        }
    }
    ...
    public function search_guest_record(Request $request)
    {
        ...
        $guests = Guests::where('first_name', 'LIKE', "%$search%")
            ...
            ->orWhere('time_in', 'LIKE', "%$searchTime%")
            ->orWhere('time_out', 'LIKE', "%$searchTime%")
            ->paginate(20);
